  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-table ref="singleTable" :data="data" highlight-current-row @current-change="handleCurrentChange" style="width: 100%" v-loading="loading">
        ...
        <el-table-column property="name" label="订单名称" fixed>
        </el-table-column>
        <el-table-column property="createTime" label="创建时间" fixed>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="centerDialogVisible" width="30%" center v-loading="dialogLoading">
      <div class="dialogMain">
        <el-form label-width="100px" :model="orderData">
          <el-form-item label="编号" v-show="!show">
            <el-input v-model="orderData.id" :disabled="!show"></el-input>
          </el-form-item>
          <el-form-item label="订单名称">
            <el-input v-model="orderData.name"></el-input>
          </el-form-item>
          <el-form-item label="创建时间" v-show="!show">
            <el-input v-model="orderData.createTime" :disabled="!show"> </el-input>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    cancel() {
      ...
      this.orderData.name = null;
      this.orderData.createTime = null;
      this.show = true;
    },
    toDo(val) {
      if (val.indexOf('新增') > -1) {
    ...
    initUpdateRoleData(id) {
      const url = '/api/order/selectOne/' + id;
      axios.get(url).then(res => {
    ...
    del() {
      if (this.currentRow == null) {
        ...
        const id = this.currentRow.id;
        this.$confirm('确定删除订单：' + this.currentRow.name + '吗？', '删除订单', {
